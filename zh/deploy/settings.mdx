...
box:
    enabled: true  # Box Runtime 总开关。false 时禁用代码沙箱、Skill 添加/编辑和 stdio MCP 等依赖 Box 的功能。
    backend: 'local'  # 'local' (Docker/nsjail), 'docker', 'nsjail', 或 'e2b'。BOX__BACKEND 可写入此配置；BOX_BACKEND 是 Runtime 级强制覆盖。
    runtime:
...
<Note>
Box 相关配置应设置在 `langbot` 服务上，使用统一的 `BOX__LOCAL__*` 环境变量。LangBot 会在启动时写入 `data/config.yaml`，并通过 INIT RPC 将 Box 配置传给 `langbot_box`。不要在 `langbot_box` 服务上设置 `LANGBOT_BOX_*` 或 `BOX__*`，这些变量不会被直接读取。`BOX_BACKEND` 仅用于有意强制 Box Runtime 后端。
</Note>
