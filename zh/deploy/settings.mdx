...
box:
    backend: 'local'  # 'local' (Docker/nsjail), 'docker', 'nsjail', 或 'e2b'。BOX_BACKEND 是 Box Runtime 强制覆盖；BOX__BACKEND 可写入此配置。
    runtime:
        endpoint: ''  # 外部 Box Runtime 基础 URL，如 'ws://127.0.0.1:5410'。留空则使用本地自动管理的运行时。
    local:
        profile: 'default'
...
<Note>
Box Runtime 本身还支持 `BOX_BACKEND` 和 `LANGBOT_BOX_LOCAL_*` 环境变量。Docker Compose 部署时通常在 `langbot` 和 `langbot_box` 两个服务中同时设置 `LANGBOT_BOX_LOCAL_HOST_ROOT`、`LANGBOT_BOX_LOCAL_DEFAULT_WORKSPACE`、`LANGBOT_BOX_LOCAL_SKILLS_ROOT` 和 `LANGBOT_BOX_LOCAL_ALLOWED_MOUNT_ROOTS`。
</Note>
