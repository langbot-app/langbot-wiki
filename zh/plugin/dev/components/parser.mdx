...
Parser 与 RAGEngine 的关系：
- **Parser** 负责将文件转换为文本（file → text）
- **RAGEngine** 负责将文本索引和检索（text → chunks → vectors）
...
RAGEngine 可通过检查 `IngestionContext.parsed_content` 来判断是否存在预解析内容：
...
RAGEngine 插件可以通过 `self.plugin.invoke_parser` 方法调用其他插件提供的解析器：
