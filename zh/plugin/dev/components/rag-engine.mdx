...
RAG 引擎（RAGEngine）组件允许插件为 LangBot 提供完整的知识库索引与检索能力。用户在 LangBot 中创建知识库时，可以选择由插件提供的 RAG 引擎来管理文档的摄取、检索和删除。

## 添加 RAG 引擎组件

单个插件中能添加任意数量的 RAG 引擎，请在插件目录执行命令 `lbp comp RAGEngine`，并根据提示输入 RAG 引擎的配置。
...
现在即会在 `components/rag_engine/` 目录下生成 `simple_rag.yaml` 和 `simple_rag.py` 文件，`.yaml` 定义了 RAG 引擎的基础信息和配置 Schema，`.py` 是该引擎的处理程序：
...
metadata:
  ...
  label:
    en_US: Simple RAG Engine  # 引擎显示名称，用于显示在 LangBot 的 UI 上，支持多语言
    zh_Hans: 简易 RAG 引擎
    ja_JP: シンプル RAG エンジン
  description:
...
与旧版 `KnowledgeRetriever` 组件使用单一 `spec.config` 不同，RAGEngine 使用两个独立的 Schema：
...
RAGEngine 可以声明自身支持的能力，LangBot 会根据能力声明决定 UI 展示和可用操作：
...
class SimpleRag(RAGEngine):
    ...
    def get_capabilities(cls) -> list[str]:
        return [
            RAGEngineCapability.DOC_INGESTION,    # 支持文档上传和处理
            RAGEngineCapability.CHUNKING_CONFIG,   # 支持自定义分块参数
            RAGEngineCapability.RERANK,            # 支持结果重排序
            RAGEngineCapability.HYBRID_SEARCH,     # 支持混合搜索（向量+关键词）
        ]
```
...
| 能力 | 说明 |
|------|------|
| `DOC_INGESTION` | 支持文档上传和处理。声明此能力后，LangBot 将在知识库详情中显示"文档"标签页 |
| `CHUNKING_CONFIG` | 支持自定义分块参数 |
| `RERANK` | 支持检索结果重排序 |
| `HYBRID_SEARCH` | 支持混合搜索（向量搜索+全文搜索） |

## 插件处理

默认会生成如下代码（`components/rag_engine/<引擎名称>.py`），您需要实现 `ingest`、`retrieve`、`delete_document` 三个核心方法。完整代码可在 [langbot-plugin-demo](https://github.com/langbot-app/langbot-plugin-demo) 的 `SimpleRAGEngine` 中找到。
...
RAGEngine 提供两个生命周期钩子，会在知识库创建和删除时被调用：
...
class FileObject(pydantic.BaseModel):
    metadata: FileMetadata    # 包含 filename, file_size, mime_type, document_id
...
RAGEngine 组件可以通过 `self.plugin` 调用 LangBot 宿主提供的 RAG 相关 API，包括嵌入模型调用、向量数据库操作和文件读取。
