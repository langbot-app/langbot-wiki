...
<Note>
`box.backend` can be set with the LangBot config override environment variable `BOX__BACKEND`. The Box Runtime also supports `BOX_BACKEND` as a runtime-only forced override, and it takes precedence over `box.backend`; in Docker Compose, set `BOX_BACKEND` on the `langbot_box` service when you need to force a backend.
</Note>
...
│                                                                  │
│  ┌─────────────────┐         ┌─────────────────────────────┐   │
│  │  Box Runtime    │         │  Sandbox Container (isolated) │   │
│  │                 │         │                             │   │
...
When LangBot runs in a Docker container, the Box Runtime runs as a separate `langbot_box` service, which mounts the host Docker socket and creates sandbox containers. `langbot_box` is an **optional service** controlled by a compose profile — it only starts when you run `docker compose --profile box up` (or `--profile all`):
