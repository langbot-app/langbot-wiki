...
These APIs are available for `RAGEngine` components to access the LangBot host's embedding models, vector database, and file storage. Access method:

- In `RAGEngine` component classes: Internal methods of the `self.plugin` object.
