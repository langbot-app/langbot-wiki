...
box:
    backend: 'local'  # 'local' (Docker/nsjail), 'docker', 'nsjail', or 'e2b'. BOX_BACKEND is a Box Runtime override; BOX__BACKEND writes this config.
    runtime:
...
<Note>
The Box Runtime itself also supports `BOX_BACKEND` and `LANGBOT_BOX_LOCAL_*` environment variables. Docker Compose deployments usually set `LANGBOT_BOX_LOCAL_HOST_ROOT`, `LANGBOT_BOX_LOCAL_DEFAULT_WORKSPACE`, `LANGBOT_BOX_LOCAL_SKILLS_ROOT`, and `LANGBOT_BOX_LOCAL_ALLOWED_MOUNT_ROOTS` on both the `langbot` and `langbot_box` services.
</Note>
