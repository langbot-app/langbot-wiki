...
box:
    ...
    backend: 'local'  # 'local' (Docker/nsjail), 'docker', 'nsjail', or 'e2b'. BOX__BACKEND writes this config; BOX_BACKEND is a Runtime-level forced override.
    runtime:
...
<Note>
Set Box-related configuration on the `langbot` service with the unified `BOX__LOCAL__*` environment variables. LangBot applies them to `data/config.yaml` on startup and sends the Box configuration to `langbot_box` through INIT RPC. Do not set `LANGBOT_BOX_*` or `BOX__*` on the `langbot_box` service; they are not read directly there. Use `BOX_BACKEND` only when intentionally forcing the Box Runtime backend.
</Note>
