...
:::tip
リクエストAPIの`list_pipeline_knowledge_bases()`や`retrieve_knowledge()`とは異なり、これらのAPIは`query_id`を必要とせず、任意のコンポーネント（Toolコンポーネントを含む）で使用できます。現在のパイプライン設定に制限されることなく、すべてのナレッジベースにアクセスできます。
:::
