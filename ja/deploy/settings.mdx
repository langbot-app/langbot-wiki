...
box:
    backend: 'local'  # 'local' (Docker/nsjail), 'docker', 'nsjail', または 'e2b'。BOX_BACKEND は Box Runtime の上書き、BOX__BACKEND はこの設定を書き込みます。
    runtime:
...
<Note>
Box Runtime 自体も `BOX_BACKEND` と `LANGBOT_BOX_LOCAL_*` 環境変数をサポートしています。Docker Compose デプロイでは通常、`langbot` と `langbot_box` の両方に `LANGBOT_BOX_LOCAL_HOST_ROOT`、`LANGBOT_BOX_LOCAL_DEFAULT_WORKSPACE`、`LANGBOT_BOX_LOCAL_SKILLS_ROOT`、`LANGBOT_BOX_LOCAL_ALLOWED_MOUNT_ROOTS` を設定します。
</Note>
