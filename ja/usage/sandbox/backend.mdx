...
<Note>
`backend`は環境変数`BOX_BACKEND`でも設定可能で、`config.yaml`より優先されます。
</Note>
...
- Docker daemonが実行中
- LangBotプロセスが`docker`コマンドを実行する権限を持つ

### 仕組み

LangBotはDocker CLIを通じてサンドボックスコンテナを作成・管理：
...
<Note>
LangBotは`docker` CLIを通じてDocker daemonと通信します。コンテナ化デプロイでは`docker.sock`をマウントする必要があります。
</Note>

### コンテナ化デプロイ

LangBotがDockerコンテナで実行される場合、ホストのDockerソケットをマウント：
...
services:
  langbot:
    ...
    volumes:
      - ./data:/app/data
      - /var/run/docker.sock:/var/run/docker.sock  # 必須：サンドボックスコンテナ作成を許可
```

<Warning>
`docker.sock`をマウントすると、コンテナが他のコンテナを作成できるようになります。信頼できる環境でのみ使用してください。
</Warning>
