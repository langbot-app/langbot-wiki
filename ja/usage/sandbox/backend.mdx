...
<Note>
`box.backend` は LangBot 設定上書き環境変数 `BOX__BACKEND` で設定できます。Box Runtime は実行時専用の強制上書きとして `BOX_BACKEND` もサポートしており、これは `box.backend` より優先されます。Docker Compose でバックエンドを強制する場合は、`BOX_BACKEND` を `langbot_box` サービスに設定してください。
</Note>
...
│                                                                  │
│  ┌─────────────────┐         ┌─────────────────────────────┐   │
│  │  Box Runtime     │         │  サンドボックスコンテナ（分離）  │   │
│  │                 │         │                             │   │
│  │  docker exec ───┼────────►│  python script.py           │   │
│  │                 │         │                             │   │
│  │  マウントディレクトリ ───┼────────►│  /workspace/                │   │
│  │                 │         │  (CPU/メモリ/ネットワーク分離)  │   │
│  └─────────────────┘         └─────────────────────────────┘   │
│                                                                  │
...
LangBot が Docker コンテナで実行される場合、Box Runtime を独立した `langbot_box` サービスとして実行します。`langbot_box` がホスト Docker socket をマウントし、サンドボックスコンテナを作成します。`langbot_box` は**オプションサービス**であり、compose profile で制御されます——`docker compose --profile box up`（または `--profile all`）で起動する必要があります：
