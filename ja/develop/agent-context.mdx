...
├── src/langbot/
│   ├── __main__.py             # 実際のエントリーポイント、CLI 引数（例：--standalone-runtime）
│   ├── pkg/                    # コアバックエンドパッケージ
...
プラグイン側のコンポーネント（`langbot-plugin-sdk` で定義）は 1 つの `BasePlugin` を中心に構成されます：
...
- **バックエンド ↔ Box ランタイム**：Box サブシステムは制御チャネル（既定ポート `5410`）で Box ランタイムに接続し、ランタイムが Docker / nsjail / E2B でサンドボックスコードを実行します。
